...
$sourceTextOutput = Join-Path $projectRoot "src\test\resources\clinical-service-test-cases.txt"

function Resolve-FunctionName {
    param([string]$TestCaseId)
    $suffix = [int]($TestCaseId.Substring($TestCaseId.Length - 3))
    if ($suffix -le 48) { return "ConsultationSaga" }
    if ($suffix -le 50) { return "AxonConfig" }
    if ($suffix -le 51) { return "ClinicalServiceApplication" }
    return "N/A"
}
...
$html = @"
<!DOCTYPE html>
<html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>Clinical Service Custom Test Status Report</title>
<style>
...
<section class="hero"><h1>Clinical Service Custom Test Status Report</h1><p class="sub">Direct PASS/FAIL view generated from JUnit XML results.</p>
