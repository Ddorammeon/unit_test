...
$sourceTextOutput = Join-Path $projectRoot "src\test\resources\insurance-service-test-cases.txt"

function Resolve-FunctionName {
    param([string]$TestCaseId)
    $suffix = [int]($TestCaseId.Substring($TestCaseId.Length - 3))
    if ($suffix -le 10) { return "InsurancePolicyService" }
    if ($suffix -le 20) { return "PatientInsuranceService" }
    if ($suffix -le 35) { return "InsuranceClaimService" }
    if ($suffix -le 43) { return "ClaimDocumentService" }
    return "N/A"
}
...
$html = @"
<!DOCTYPE html>
<html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>Insurance Service Custom Test Status Report</title>
<style>
...
<section class="hero"><h1>Insurance Service Custom Test Status Report</h1><p class="sub">Direct PASS/FAIL view generated from JUnit XML results.</p>
