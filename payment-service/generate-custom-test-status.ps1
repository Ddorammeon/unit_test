...
$sourceTextOutput = Join-Path $projectRoot "src\test\resources\payment-service-test-cases.txt"

function Resolve-FunctionName {
    param([string]$TestCaseId)
    $suffix = [int]($TestCaseId.Substring($TestCaseId.Length - 3))
    switch ($suffix) {
        { $_ -le 13 } { return "initiatePayment()" }
        14 { return "getPaymentStatusOfInvoice()" }
        15 { return "getPaymentById()" }
        { $_ -ge 16 -and $_ -le 17 } { return "updatePayment()" }
        18 { return "validateInvoice()" }
        19 { return "deletePayment()" }
        { $_ -ge 20 -and $_ -le 22 } { return "initiatePayment()" }
        23 { return "handlePayOSWebhook()" }
        24 { return "validateInvoice()" }
        25 { return "handlePaymentCallback()" }
        { $_ -ge 26 -and $_ -le 27 } { return "on(PaymentInitiatedEvent)" }
        28 { return "on(PaymentProcessedEvent)" }
        { $_ -ge 29 -and $_ -le 30 } { return "on(PaymentFailedEvent)" }
        { $_ -ge 31 -and $_ -le 32 } { return "on(InvoiceCancelledEvent)" }
        33 { return "onTimeout()" }
        34 { return "on(PaymentProcessedEvent)" }
        { $_ -ge 35 -and $_ -le 39 } { return "on(PaymentStatusUpdatedEvent)" }
        40 { return "handle(CreatePaymentCommand)" }
        { $_ -ge 41 -and $_ -le 42 } { return "handle(UpdatePaymentStatusCommand)" }
        43 { return "PaymentAggregate(...)" }
        44 { return "applyUpdatePaymentStatus()" }
        45 { return "on(PaymentInitiatedEvent)" }
        46 { return "on(PaymentStatusUpdatedEvent)" }
        47 { return "PaymentMethod#getDescription()" }
        48 { return "PaymentMethod#getDescription()" }
        49 { return "PaymentStatus#getDescription()" }
        50 { return "Payment.builder()" }
        default { return "N/A" }
    }
}
...
Get-ChildItem -Path $testResultsDir -Filter "TEST-*.xml" | Sort-Object Name | ForEach-Object {
    ...
    $testSuiteNode = if ($suite.testsuite) { $suite.testsuite } else { $suite.DocumentElement }

    foreach ($testcase in @($testSuiteNode.testcase)) {
        if ($null -eq $testcase -or $testcase.name -notlike "* - *") {
            continue
        }
        ...
        $functionName = Resolve-FunctionName -TestCaseId $testCaseId
        $displayName = "$testCaseId - $objective"
...
$html = @"
<!DOCTYPE html>
<html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>Payment Service Custom Test Status Report</title>
<style>
...
<section class="hero"><h1>Payment Service Custom Test Status Report</h1><p class="sub">Direct PASS/FAIL view generated from JUnit XML results.</p>
